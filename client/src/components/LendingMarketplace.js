...
const LendingMarketplace = () => {
	return (
		<TableContainer component={Paper}>
			<Table sx={{ minWidth: 650, border: '1px solid #e1e1e1' }} aria-label="simple table">
				<TableHead>
					<TableRow>
						{TABLEHEADERS.map((k, i) => (
							<TableCell component="th" align="center" key={i}>
								...
								{k.desc ?
									...
									>
										<HelpOutlineIcon sx={{ fontSize: 'small', cursor: 'pointer', marginLeft: '5px' }} />
									</HtmlTooltip>
								: "" }
							</TableCell>
						))}
							...
							<TableCell align="center">
								<Link to={`/portfolio/${row.address}`} style={{textDecoration: 'none', cursor: 'pointer'}}>
									{row.status === 'Full' ?
										...
										: <Chip label={row.status} sx={{backgroundColor: '#2b8cbe', color: '#fff'}} />
									}
								</Link>
							</TableCell>
						</TableRow>
