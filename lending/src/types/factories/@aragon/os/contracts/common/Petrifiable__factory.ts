/* Autogenerated file. Do not edit manually. */

/* tslint:disable */

/* eslint-disable */
import type {
  Petrifiable,
  PetrifiableInterface,
} from "../../../../../@aragon/os/contracts/common/Petrifiable";
import type { Provider, TransactionRequest } from "@ethersproject/providers";
import { Signer, utils, Contract, ContractFactory, Overrides } from "ethers";

const _abi = [
  {
    constant: true,
    inputs: [],
    name: "hasInitialized",
    outputs: [
      {
        name: "",
        type: "bool",
      },
    ],
    payable: false,
    stateMutability: "view",
    type: "function",
  },
  {
    constant: true,
    inputs: [],
    name: "getInitializationBlock",
    outputs: [
      {
        name: "",
        type: "uint256",
      },
    ],
    payable: false,
    stateMutability: "view",
    type: "function",
  },
  {
    constant: true,
    inputs: [],
    name: "isPetrified",
    outputs: [
      {
        name: "",
        type: "bool",
      },
    ],
    payable: false,
    stateMutability: "view",
    type: "function",
  },
];

const _bytecode =
  "";

type PetrifiableConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: PetrifiableConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class Petrifiable__factory extends ContractFactory {
  constructor(...args: PetrifiableConstructorParams) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
  }

  override deploy(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): Promise<Petrifiable> {
    return super.deploy(overrides || {}) as Promise<Petrifiable>;
  }
  override getDeployTransaction(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(overrides || {});
  }
  override attach(address: string): Petrifiable {
    return super.attach(address) as Petrifiable;
  }
  override connect(signer: Signer): Petrifiable__factory {
    return super.connect(signer) as Petrifiable__factory;
  }

  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): PetrifiableInterface {
    return new utils.Interface(_abi) as PetrifiableInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): Petrifiable {
    return new Contract(address, _abi, signerOrProvider) as Petrifiable;
  }
}
